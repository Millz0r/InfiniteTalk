...
def generate(args):
    ...
    for job_item in input_data['items']:
        ...
        if args.scene_seg and is_video(job_item['cond_video']):
            ...
            if len(time_list)==0:
                conds_list.append([job_item['cond_video']])
                conds_list.append([job_item['cond_audio']['person1']])
                if len(job_item['cond_audio'])==2:
                    conds_list.append([job_item['cond_audio']['person2']])
            else:
                audio1_list = split_wav_librosa(job_item['cond_audio']['person1'], time_list, args.audio_save_dir)
                conds_list.append(cond_list)
                conds_list.append(audio1_list)
                if len(job_item['cond_audio'])==2:
                    audio2_list = split_wav_librosa(job_item['cond_audio']['person2'], time_list, args.audio_save_dir)
        ...
        if len(job_item['cond_audio'])==2:
            conds_list.append([job_item['cond_audio']['person2']])
        if len(job_item['cond_audio'])==2:
            new_human_speech1, new_human_speech2, sum_human_speechs = audio_prepare_multi(job_item['cond_audio']['person1'], job_item['cond_audio']['person2'], job_item['audio_type'])
            sum_audio = os.path.join(args.audio_save_dir, 'sum_all.wav')
            sf.write(sum_audio, sum_human_speechs, 16000)
            job_item['video_audio'] = sum_audio
        else:
            human_speech = audio_prepare_single(job_item['cond_audio']['person1'])
            sum_audio = os.path.join(args.audio_save_dir, 'sum_all.wav')
        ...
        for idx, items in enumerate(zip(*conds_list)):
            print(items)
            input_clip = {}
            input_clip['prompt'] = job_item['prompt']
            input_clip['cond_video'] = items[0]

            if 'audio_type' in job_item:
                input_clip['audio_type'] = job_item['audio_type']
            if 'bbox' in job_item:
                input_clip['bbox'] = job_item['bbox']
            cond_audio = {}
            if args.audio_mode=='localfile':
                if len(job_item['cond_audio'])==2:
                    new_human_speech1, new_human_speech2, sum_human_speechs = audio_prepare_multi(items[1], items[2], job_item['audio_type'])
                ...
                elif len(job_item['cond_audio'])==1:
